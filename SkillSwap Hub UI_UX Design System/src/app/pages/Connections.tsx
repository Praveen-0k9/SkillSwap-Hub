...
const tabs = ["All", "Online", "Suggestions"] as const;
    ...
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">{mockUsers.length} connections in your network</p>
        </div>
      </div>
        ...
        <div className="flex gap-1">
          {tabs.map((t) => (
            ...
            >
              {t}
              {t === "Online" && (
                <span className="ml-1.5 inline-flex items-center justify-center w-4 h-4 rounded-full bg-green-400/15 text-green-400 text-xs font-bold">
                  {online.length}
                </span>
              )}
            </button>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {user.skills.slice(0, 3).map((skill) => (
                      <span key={skill} className="inline-block px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-xs">
                        {skill}
                      </span>
                    ))}
                  </div>
                  ...
                  <div className="flex gap-2">
                    <Link
                      to="/chat"
                      className="flex-1 inline-flex items-center justify-center gap-1.5 border border-border rounded-md py-1.5 text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors"
                    >
                      <MessageSquare size={12} />
                      Message
                    </Link>
                    <button className="p-1.5 border border-border rounded-md text-muted-foreground hover:text-red-400 hover:border-red-400/30 transition-colors flex-shrink-0">
                      <UserMinus size={13} />
                    </button>
                  </div>
                </div>
            ...
            <div key={user.id} className="bg-card border border-border rounded-lg p-5">
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-4">
                    {user.skills.map((s) => (
                      <span key={s} className="inline-block px-2 py-0.5 rounded-full bg-muted text-muted-foreground text-xs">{s}</span>
                    ))}
                  </div>
                  <button className="w-full inline-flex items-center justify-center gap-1.5 bg-primary text-white rounded-md py-1.5 text-xs font-medium hover:bg-primary/90 transition-colors">
                    <UserPlus size={12} />
                    Connect
                  </button>
                </div>
